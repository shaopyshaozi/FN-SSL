...
RESPEAKER_DATA_DIR = os.path.join(SCRIPT_DIR, 'Respeaker')
RESPEAKER_RESULTS_DIR = os.path.join(SCRIPT_DIR, 'inference_results_respeaker')
...
class InferenceDataset(Dataset):
    def __init__(self, data_dir):
        ...
        for path in sorted(wav_paths):
            ...
            match = re.match(r'(test\d+)', file_stem)
            test_id = match.group(1) if match else file_stem
            if test_id not in selected:
                selected[test_id] = path
...
class MyModel(LightningModule):
    ...
    def predict_step(self, batch, batch_idx: int):
        ...
        os.makedirs(RESPEAKER_RESULTS_DIR, exist_ok=True)
        np.save(os.path.join(RESPEAKER_RESULTS_DIR, str(batch_idx) + '_doaest'), doa_est.cpu().numpy())
        np.save(os.path.join(RESPEAKER_RESULTS_DIR, str(batch_idx) + '_vadest'), vad_est.cpu().numpy())
        np.save(os.path.join(RESPEAKER_RESULTS_DIR, str(batch_idx) + '_ipd'), pred_ipd.detach().cpu().numpy())
        np.save(os.path.join(RESPEAKER_RESULTS_DIR, str(batch_idx) + '_spectrum'), pred_spatial_spectrum.detach().cpu().numpy())
